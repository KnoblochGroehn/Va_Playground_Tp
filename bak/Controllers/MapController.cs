...
namespace MapAppMVC.Controllers
{
    ...
    public class MapController : ControllerBase
    {
        ...
        private static string SafeText(string? s)
        {
            ...
            // remove chars that can break HTML/attributes
        ...
        static MapController()
        {
            ...
            for (int i = 0; i < 160; i++)
            {
                ...
                _polygons[id].Layer = _rng.Next(2);
            }
        ...
        public ActionResult<IEnumerable<object>> GetCirclesDelta([FromQuery] double? lon = null, [FromQuery] double? lat = null, [FromQuery] double km = 25, [FromQuery] int n = 10, [FromQuery] int debug = 0)
        {
            ...
            foreach (var c in list)
            {
                ...
                // occasionally flip layer
                if (_rng.NextDouble() < 0.06) c.Layer = 1 - (c.Layer % 2);
        ...
        public ActionResult<IEnumerable<object>> GetIconsDelta([FromQuery] double? lon = null, [FromQuery] double? lat = null, [FromQuery] double km = 25, [FromQuery] int n = 10, [FromQuery] int debug = 0)
        {
            ...
            foreach (var m in list)
            {
                ...
                // occasionally flip layer
                if (_rng.NextDouble() < 0.06) m.Layer = 1 - (m.Layer % 2);
        ...
        public ActionResult<IEnumerable<object>> GetPolygonsDelta([FromQuery] double? lon = null, [FromQuery] double? lat = null, [FromQuery] double km = 25, [FromQuery] int n = 6, [FromQuery] int debug = 0)
        {
            ...
            foreach (var p in list)
            {
                if (_rng.NextDouble() < 0.12)
                {
                    updates.Add(new { id = p.Id, Kill = true, layer = p.Layer });
                ...
                // keep / flip layer
                np.Layer = p.Layer;
                if (_rng.NextDouble() < 0.06) np.Layer = 1 - (np.Layer % 2);
        ...
        private static PolygonResponse RandomPolygon(string id, double lon, double lat)
        {
            ...
            double baseR = 0.015 + _rng.NextDouble() * 0.25; // deg-ish
        ...
        private static readonly IconPreset[] _demo5IconPresets = new[]
        {
            ...
            // Signal / Wifi / Broadcast / Antenna
            new IconPreset("Signal",       "\uf012", 26, "middle"),
            new IconPreset("Wifi",         "\uf1eb", 26, "middle"),
            new IconPreset("Broadcast",    "\uf519", 26, "middle"),
        ...
        private static void InitDemo5()
        {
            ...
            for (int i = 0; i < 260; i++)
            {
                ...
                _demo5Icons[id] = new IconResponse
                {
                    ...
                    Html = $"<div class='fw-semibold'>Demo5 Icon {i + 1}</div><div class='text-muted'>Mobile/Radio</div>",
            ...
            // Polygons (low fill, clear outline)
            for (int i = 0; i < 60; i++)
            {
                var id = $"d5p{i + 1}";
